...
- **llama.cpp tool calling:** Gemma 4 requires build b8665+ (PR #21418). Llama 4 Scout is incompatible (no pythonic parser). Channel/reasoning models (Gemma 4, GLM, Qwen 3.x) need `--jinja --reasoning-format deepseek` on llama-server to keep `content` clean for tool calling. **Verified (build 9870, [lprsoft-lab/llm-local#5](https://github.com/lprsoft-lab/llm-local/issues/5)):** `none` LEAVES thoughts unparsed in `content` (leaks `<channel|>`, degenerates to a stall — this caused the `gemma4_26b_mlx` failure); `deepseek` moves them to `reasoning_content` and yields `finish_reason=tool_calls`. Applied globally on the Mac Studio via `llama_common` (commit `01a4963`). Caveat (updated): `mlx_lm.server` (the `mlm-` MLX path) has no `--reasoning-format` flag, so *historically* the MLX path leaked channel tokens (this caused the original `gemma4_26b_mlx` failure) and only the llama.cpp (`clm-`) backend could take this fix. **No longer true as of `mlx_lm` 0.31.3**, which separates thinking into `reasoning_content` natively via the chat template — verified on `mlm-gemma-4-26B-A4B-it-OptiQ-4bit` (tool-call probe: `finish_reason=tool_calls`, clean empty `content`; a 67-step agentic benchmark loop produced zero channel-leak markers). Both backends now keep `content` clean; older `mlx_lm` versions still leak. (Older docs/READMEs still say `none`; that avoided a hard autoparser crash on older builds but does not strip the tokens.)
